...
let helloBtn = document.querySelector('#say-hello-button');
...
const changeColor = () => {
    ...
    helloBtn.classList.add('invert-text')
}
const removeColor = () => {
    console.log('john')

    helloBtn.classList.remove('invert-text')
}
...
helloBtn.addEventListener('mouseout', removeColor)
...
let sayHelloButton = helloBtn.addEventListener('click', sayHello)
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((promise) => {console.log(promise.data)}).catch((err) => {
        console.log(err)
    } )
}
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/random').then((promise) => {
...
repeatEl.textContent = promise.data;
...
document.getElementById('query-button').addEventListener('click', sendAQuery)
